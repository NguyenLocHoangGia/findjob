...
public interface JobRepository extends JpaRepository<Job, Long> {
        ...
        @Query("SELECT j FROM Job j WHERE j.isActive = true AND j.status = 'APPROVED' " +
                        "AND (:keyword IS NULL OR :keyword = '' OR LOWER(j.title) LIKE LOWER(CONCAT('%', :keyword, '%')) OR LOWER(j.companyName) LIKE LOWER(CONCAT('%', :keyword, '%'))) "
                        +
                        "AND (:location IS NULL OR :location = '' OR LOWER(j.location) LIKE LOWER(CONCAT('%', :location, '%')) OR LOWER(:location) LIKE LOWER(CONCAT('%', j.location, '%'))) "
                        +
        ...
        List<Job> findByIsActiveTrueAndStatusOrderByCreatedAtDesc(String status);
        ...
        // 1. Dành cho Admin Header: Đếm số lượng tin chờ duyệt
