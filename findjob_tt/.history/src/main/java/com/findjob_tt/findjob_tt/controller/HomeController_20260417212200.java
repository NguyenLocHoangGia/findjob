...
public class HomeController {
    ...
    public String showCandidateHomepage(
        ...
        populateHomeData(keyword, location, categoryId, model, false);
        ...
        populateHomeData(keyword, location, categoryId, model, true);
        return "candidate/homePage";
        }
        ...
        List<Job> displayedHotJobs = isAllJobsPage ? hotJobs : hotJobs.stream().limit(3).toList();
        List<Job> displayedRecentJobs = isAllJobsPage ? jobs : jobs.stream().limit(3).toList();
        ...
        model.addAttribute("hasMoreRecentJobs", jobs.size() > 3 && !isAllJobsPage);
