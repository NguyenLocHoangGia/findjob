...
public class RecruiterController {
    ...
    public String saveJob(
        ...
        job.setStatus(com.findjob_tt.findjob_tt.model.Status.PENDING);
        ...
        redirectAttributes.addFlashAttribute("successMsg", "Đăng tin thành công! Vui lòng chờ Admin duyệt.");
        return "redirect:/recruiter/dashboard";
    }
    ...
    public String toggleJobStatus(@PathVariable Long id, Principal principal, RedirectAttributes redirectAttributes) {
        ...
        if (job != null && job.getUser().getEmail().equals(principal.getName())) {
            ...
            // Logic toggle: PENDING -> CLOSED, CLOSED -> PENDING, APPROVED -> CLOSED
            if (currentStatus == Status.PENDING || currentStatus == Status.CLOSED) {
                job.setStatus(currentStatus == Status.PENDING ? Status.CLOSED : Status.PENDING);
