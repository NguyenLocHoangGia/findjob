...
public class RecruiterController {
    ...
    public String showRegisterCompanyForm(Principal principal, Model model,
        ...
        if (company != null) {
            // 1. ĐÃ DUYỆT: Đá thẳng vào Dashboard, không cho xem form đăng ký nữa
            if ("APPROVED".equals(company.getStatus())) {
                return "redirect:/recruiter/dashboard";
            }

            // 2. BỊ TỪ CHỐI & XIN LÀM LẠI: Cho phép mở lại Form để ghi đè dữ liệu mới
            if ("REJECTED".equals(company.getStatus()) && "true".equals(retry)) {
                model.addAttribute("company", company); // Gửi data cũ ra để điền sẵn vào form
                return "recruiter/register-company";
            }

            // 3. ĐANG CHỜ hoặc BỊ TỪ CHỐI (Mặc định): Đẩy sang trang Thông báo trạng thái
        ...
        // 4. CHƯA CÓ GÌ: Lần đầu tiên bấm vào -> Mở form đăng ký trống
        return "recruiter/register-company";
    }
