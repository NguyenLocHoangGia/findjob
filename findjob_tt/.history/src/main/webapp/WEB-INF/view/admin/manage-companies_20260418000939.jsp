...
<body>
    ...
    <div class="container my-5" style="padding-top: 20px;">
        ...
        <c:if test="${not empty successMsg}">
            <div class="alert alert-success fw-bold alert-dismissible fade show shadow-sm"><i class="bi bi-check-circle-fill me-2"></i>${successMsg}<button type="button" class="btn-close" data-bs-dismiss="alert"></button></div>
        </c:if>
        <c:if test="${not empty errorMsg}">
            <div class="alert alert-danger fw-bold alert-dismissible fade show shadow-sm"><i class="bi bi-x-circle-fill me-2"></i>${errorMsg}<button type="button" class="btn-close" data-bs-dismiss="alert"></button></div>
        </c:if>

        <div class="d-flex justify-content-between align-items-center mb-4 mt-2">
            <div>
                <h2 class="fw-bold mb-1 text-dark"><i class="bi bi-building-check text-primary me-2"></i>Kiểm duyệt Doanh nghiệp</h2>
                        ...
                        <tbody id="company-tbody">
                            <c:forEach var="company" items="${companies}" varStatus="loop">
                                ...
                                <tr class="company-row" 
                                    ...
                                    <td class="text-center align-middle">${currentPage * 5 + loop.index + 1}</td>
                                    
                                    <td class="ps-4 align-middle">
                                        <div class="d-flex align-items-center">
                                            <img src="${not empty company.companyLogo ? '/uploads/company_logos/'.concat(company.companyLogo) : 'https://via.placeholder.com/45?text=Logo'}" class="logo-sm me-3" alt="Logo">
                                            <div>
                                                <div class="fw-bold text-dark mb-1">${company.companyName}</div>
                                                <div class="small text-muted"><i class="bi bi-person me-1"></i>${company.user.email}</div>
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <td class="align-middle">
                                        <c:choose>
                                            <c:when test="${not empty company.businessLicense}">
                                                <a href="/uploads/business_licenses/${company.businessLicense}" target="_blank" class="btn btn-sm btn-outline-primary rounded-pill">
                                                    <i class="bi bi-file-earmark-text me-1"></i> Xem Giấy phép
                                                </a>
                                            </c:when>
                                            <c:otherwise>
                                                <span class="text-danger small fst-italic"><i class="bi bi-exclamation-circle me-1"></i>Thiếu GPKD</span>
                                            </c:otherwise>
                                        </c:choose>
                                    </td>
                                    ...
                                    <td class="text-center align-middle">
                                        <c:choose>
                                            <c:when test="${company.status == 'APPROVED'}">
                                                <span class="badge bg-success-subtle text-success border border-success border-opacity-25 px-3 py-2 rounded-pill w-100">Đã duyệt</span>
                                            </c:when>
                                            <c:when test="${company.status == 'REJECTED'}">
                                                <span class="badge bg-danger-subtle text-danger border border-danger border-opacity-25 px-3 py-2 rounded-pill w-100">Từ chối</span>
                                            </c:when>
                                            <c:otherwise>
                                                <span class="badge bg-warning-subtle text-warning border border-warning border-opacity-25 px-3 py-2 rounded-pill w-100">Đang chờ</span>
                                            </c:otherwise>
                                        </c:choose>
                                    </td>
                                    ...
                                    <td class="text-center align-middle">
                                        <c:if test="${company.status == 'PENDING' || empty company.status}">
                                            <div class="d-flex justify-content-center gap-2">
                                                <form action="/admin/companies/${company.id}/approve" method="POST" class="m-0 confirm-action" data-confirm-message="Xác nhận CẤP QUYỀN Nhà tuyển dụng cho công ty này?">
                                                    <input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}"/>
                                                    <button type="submit" class="btn btn-sm btn-success rounded-pill px-3 shadow-sm" title="Duyệt"><i class="bi bi-check-lg"></i></button>
                                                </form>
                                                <form action="/admin/companies/${company.id}/reject" method="POST" class="m-0 confirm-action" data-confirm-message="Bạn từ chối hồ sơ này?">
                                                    <input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}"/>
                                                    <button type="submit" class="btn btn-sm btn-danger rounded-pill px-3 shadow-sm" title="Từ chối"><i class="bi bi-x-lg"></i></button>
                                                </form>
                                            </div>
                                        </c:if>
    ...
    <script>
        ...
        // Mặc định chạy lọc một lần khi vừa vào trang (vì select box đang để mặc định là PENDING)
