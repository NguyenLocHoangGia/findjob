...
<body>
    ...
    <div class="container my-5">
        ...
        <div class="d-flex justify-content-between align-items-center mb-3">
            <h5 class="mb-0 text-muted fs-6">Danh sách công việc</h5>
            <form action="/admin/jobs" method="GET" class="d-flex align-items-center">
                <label for="statusFilter" class="me-2 fw-medium text-muted mb-0 small">Lọc theo:</label>
                <select name="status" id="statusFilter" class="form-select form-select-sm shadow-none" onchange="this.form.submit()" style="width: auto; cursor: pointer;">
                    ...
                    <option value="APPROVED" ${currentStatus == 'APPROVED' ? 'selected' : ''}>Đang hiển thị</option>
                    <option value="REJECTED" ${currentStatus == 'REJECTED' ? 'selected' : ''}>Đã đóng</option>
                </select>
            </form>
        </div>
...
<c:if test="${job.status == 'PENDING' || (job.status == 'APPROVED' && job.isActive) || empty job.status}">
    <form action="/admin/jobs/reject" method="POST" class="m-0 confirm-action" data-confirm-message="Bạn có chắc chắn muốn đóng/từ chối tin này không?">
        <input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}"/>
        <input type="hidden" name="jobId" value="${job.id}">
        
        <input type="hidden" name="page" value="${currentPage}">
        
        <button type="submit" class="btn btn-sm btn-danger" title="Từ chối/Đóng tin">
