...
import com.findjob_tt.findjob_tt.service.JobRequiredSkillService;
...
public class RecruiterController {
    ...
    @Autowired
    private ParsedCvSkillRepository parsedCvSkillRepository;

    @Autowired
    private JobRequiredSkillService jobRequiredSkillService;
    ...
    private String normalizeSalaryInput(String salaryInput) {
        if (salaryInput == null) {
            return null;
        }

        String trimmed = salaryInput.trim();
        if (trimmed.isEmpty()) {
            return null;
        }

        String normalized = trimmed.toLowerCase(Locale.ROOT);
        if (normalized.equals("thỏa thuận") || normalized.equals("thoa thuan")) {
            return "Thỏa thuận";
        }
    ...
    public String showCandidatesByJob(@PathVariable("id") Long jobId, Model model, Principal principal) {
        ...
        List<Application> applications = job.getApplications() != null ? job.getApplications() : Collections.emptyList();
        Map<Long, CvScore> cvScoreMap = new HashMap<>();
        List<Long> applicationIds = applications.stream()
                .map(Application::getId)
                .toList();
        if (!applicationIds.isEmpty()) {
            cvScoreMap = cvScoreRepository.findByApplication_IdIn(applicationIds).stream()
                    .collect(Collectors.toMap(score -> score.getApplication().getId(), Function.identity(), (a, b) -> a));
        }

        model.addAttribute("job", job);
        model.addAttribute("applications", applications);
        model.addAttribute("cvScoreMap", cvScoreMap);
        return "recruiter/job-candidates";
    }

    @GetMapping("/recruiter/application/{id}/cv-score")
    public String showCvScoreDetail(@PathVariable("id") Long applicationId, Model model, Principal principal) {
        String email = getEmailFromPrincipal(principal);
        if (email == null || email.isBlank()) {
            return "redirect:/login";
        }
        ...
        if (application == null || application.getJob() == null || application.getJob().getUser() == null
                || !email.equals(application.getJob().getUser().getEmail())) {
            return "redirect:/recruiter/dashboard?error=access_denied";
        }

        CvScore score = cvScoreRepository.findByApplication_Id(applicationId).orElse(null);
        List<ParsedCvSkill> skills = Collections.emptyList();
        if (score != null && score.getParsedCv() != null && score.getParsedCv().getId() != null) {
            skills = parsedCvSkillRepository.findByParsedCv_Id(score.getParsedCv().getId());
        }

        model.addAttribute("application", application);
        model.addAttribute("job", application.getJob());
        model.addAttribute("score", score);
        model.addAttribute("parsedCv", score != null ? score.getParsedCv() : null);
        model.addAttribute("skills", skills);
        return "recruiter/cv-score-detail";
    }
    ...
    public String saveJob(
            ...
            @RequestParam(value = "requiredSkillNames", required = false) List<String> requiredSkillNames,
            @RequestParam(value = "requiredSkillTypes", required = false) List<String> requiredSkillTypes,
            @RequestParam(value = "requiredSkillMinYears", required = false) List<String> requiredSkillMinYears,
            @RequestParam(value = "requiredSkillWeights", required = false) List<String> requiredSkillWeights,
            @RequestParam(value = "requiredSkillMustHaves", required = false) List<String> requiredSkillMustHaves,
            Principal principal,
            RedirectAttributes redirectAttributes) {
        ...
        String normalizedSalary = normalizeSalaryInput(job.getSalary());
        if (normalizedSalary == null) {
            redirectAttributes.addFlashAttribute("errorMsg", "Mức lương chỉ được nhập số không âm hoặc 'Thỏa thuận'.");
            return "redirect:/recruiter/job/add";
        }
        job.setSalary(normalizedSalary);
        ...
        jobRequiredSkillService.replaceSkills(
                job,
                requiredSkillNames,
                requiredSkillTypes,
                requiredSkillMinYears,
                requiredSkillWeights,
                requiredSkillMustHaves);
    ...
    public String showEditJobForm(@PathVariable Long id, Principal principal, Model model,
        ...
        model.addAttribute("job", job);
        model.addAttribute("jobRequiredSkills", jobRequiredSkillService.findByJobId(job.getId()));
    ...
    public String updateJob(@ModelAttribute Job formJob,
        ...
        if (existingJob != null && existingJob.getUser().getEmail().equals(email)) {
            ...
            String normalizedSalary = normalizeSalaryInput(formJob.getSalary());
            if (normalizedSalary == null) {
                redirectAttributes.addFlashAttribute("errorMsg",
                        "Mức lương chỉ được nhập số không âm hoặc 'Thỏa thuận'.");
                return "redirect:/recruiter/edit-job/" + formJob.getId();
            }
