...
import com.findjob_tt.findjob_tt.service.cv.CvScoringAsyncService;
...
public class CandidateController {
    ...
    @Autowired
    private CvScoringAsyncService cvScoringAsyncService;
    ...
    // Hồ sơ hoàn chỉnh để được ứng tuyển: họ tên, số điện thoại, địa chỉ, mô tả và
    // CV
    ...
    public String applyJob(@RequestParam Long jobId, Principal principal, RedirectAttributes ra) {
        ...
        if (user.getCandidateProfile() != null) {
            newApp.setCvFileName(user.getCandidateProfile().getCvUrl());
        }
        applicationRepository.save(newApp);
        cvScoringAsyncService.scoreApplicationAsync(newApp.getId());
